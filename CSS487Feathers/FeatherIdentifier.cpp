...
/*
Train
Precondition: The FeatherIdentifier is untrained
	arguments:
		trainingFile - the file that indicates training variables and
		verify - whether to verify how well the FeatherIdentifier has trained
			by testing the training data against itself
		verbose - whether or not to give extra UI feedback
Postcondition: The FeatherIdentifier has a trained BOW + SVM, or process has 
	failed
	returns:
		success state (T/F)
*/
bool FeatherIdentifier::Train(const string &trainingFile, bool verify, bool verbose)
{
	...
	cout << "-----------------------------------------" << endl;
	if (!CreateDictionary(detector, extractor))
	{
...
/*
Train
Precondition: The FeatherIdentifier has a trained BOW + SVM
	arguments:
		testingFile - the file that indicates imags to be tested
		verbose - whether or not to give extra UI feedback
Postcondition: The FeatherIdentifier has Identified all of the testing
	data, or the process has failed
	returns:
		success state (T/F)
*/
...
/*
Save
Precondition: The FeatherIdentifier is trained
	arguments:
		saveName - the name to save the dictionary and SVM as
Postcondition: The BOW dictionary and SVM are saved to disk
	returns:
		success state (T/F)
*/
...
/*
Save
Precondition: The FeatherIdentifier is not trained, there are dictionary
	and SVM files under the name indicated
	arguments:
		saveName - the name of the dictionary and SVM
Postcondition: The BOW dictionary and SVM are loaded
	returns:
		success state (T/F)
*/
...
/*
CreateDictionary
Precondition: The FeatherIdentifier is untrained, detector and extractor
	are valid FeatureDetector & DescriptorExtractors
	arguments:
		detector - a pointer to the FeatureDetector to be used
		extractor - a pointer to the DescriptorExtractor to be used
Postcondition: A dictionary/vocabulary is generated by the BOWKMeansTrainer
	returns:
		success state (T/F)
*/
bool FeatherIdentifier::CreateDictionary(Ptr<FeatureDetector> &detector, Ptr<DescriptorExtractor> &extractor)
{
...
/*
CalculateHistograms
Precondition: The FeatherIdentifier is untrained, detector and extractor are valid
	FeatureDetector & DescriptorExtractor
arguments:
	detector - a pointer to the FeatureDetector to be used
	extractor - a pointer to the DescriptorExtractor to be used
	outSamples - the histograms for all of the training images 
	outLabels - the labels for each histogram (which category it belongs to)
Postcondition: A dictionary/vocabulary is generated by the BOWKMeansTrainer
	returns:
		success state (T/F)
*/
...
/*
CalculateHistograms
Precondition: samples and labels contain valid data
	arguments:
		samples - the histograms for all of the training images
		labels - the labels for each histogram (which category it belongs to)
Postcondition: The SVM is trained
	returns:
		success state (T/F)
*/
...
/*
TestSVM - Tests if the SVM is
Precondition: The SVM is trained
	arguments:
		detector - the detector to use
		extractor - the extractor to use
		trainSets - the trainingSets used to train the SVM
		testSets - the sets to test agains
		verbose - whether or not to give extra UI feedback
Postcondition: The results of testing the images in testSets are displayed to the screen
	returns:
		success state (T/F)
*/
